    ...
    public class SecurityConfig {
        ...
        public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
            ...
            .authorizeHttpRequests(authorize -> authorize
                .requestMatchers("/api/user/login", "/api/user/signUp/**", "/api/user/sendCode", "/api/qna/**").permitAll() // 로그인과 회원가입 API는 인증 없이 접근 가능하도록 설정합니다.
        ...
        public static class JwtTokenFilter extends OncePerRequestFilter {
            ...
            protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain) throws ServletException, IOException {
                ...
                if (token != null && jwtTokenProvider.validateToken(token)) { // 토큰이 유효한 경우
                    ...
                    Integer userId = jwtTokenProvider.getUserIdFromToken(token);  // 토큰에서 userId를 추출합니다.
                    request.setAttribute("userId", userId);
                    System.out.println(userId);

                    String userName = jwtTokenProvider.getUserNameFromToken(token);  // 토큰에서 userId를 추출합니다.
